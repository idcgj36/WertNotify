if _G.WertUiAnimation == nil then
...
function WertUi:Notify(settings)
	...
	if StrokeThickness > 0 then
		...
		UIStroke.Color = StrokeColors[1]

		local gradient = Instance.new("UIGradient", UIStroke)
		local seq = {}
		if #StrokeColors == 1 then
			seq = {ColorSequenceKeypoint.new(0, StrokeColors[1]), ColorSequenceKeypoint.new(1, StrokeColors[1])}
		else
			for i,color in ipairs(StrokeColors) do
				table.insert(seq, ColorSequenceKeypoint.new((i-1)/(#StrokeColors-1), color))
			end
		end
		gradient.Color = ColorSequence.new(seq)
		gradient.Rotation = 0

		if _G.WertUiAnimation and #StrokeColors > 1 then
			task.spawn(function()
				while Frame.Parent do
					local tween = TweenService:Create(gradient,TweenInfo.new(2,Enum.EasingStyle.Linear,Enum.EasingDirection.InOut),{Rotation=360})
					tween:Play()
	...
	local tweenIn = TweenService:Create(Frame,TweenInfo.new(0.4,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Position=UDim2.new(0,0,0,0),BackgroundTransparency=Transparent})
	tweenIn:Play()

	local function removeNotification()
		local tweenOut = TweenService:Create(Frame,TweenInfo.new(0.4,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Position=UDim2.new(1,500,0,0),BackgroundTransparency=1})
		tweenOut:Play()
